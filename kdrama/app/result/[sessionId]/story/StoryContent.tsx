...
import type { Session, Episode } from "@/lib/types";
import { generateStory } from "@/lib/clientApi";
...
export default function StoryContent() {
  ...
  async function generate() {
    ...
    try {
      ...
      const updated: Session = { ...session, episodes };
    ...
    <main className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-white">
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <h2 className="text-xl font-bold text-gray-800 mb-1">13話ストーリー</h2>
          <p className="text-sm text-gray-500 mb-6">
            {session.persona?.dominantType} のペルソナが主人公の韓ドラ風ストーリー
          </p>
          ...
          {!session.episodes ? (
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={generate}
                disabled={loading}
                className="w-full mt-4 py-3 border border-purple-200 text-purple-600 text-sm font-semibold rounded-xl hover:bg-purple-50 transition-colors disabled:opacity-50"
              >
                {loading ? "再生成中..." : "再生成する"}
              </button>
            </div>
