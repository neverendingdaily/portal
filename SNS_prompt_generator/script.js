...
function buildPrompt(mediaType, platform, idea) {
  ...
  const seg1 = `[${rule.aspect_ratio}] ${mediaDescriptor}`;
  const seg2 = expandIdea(idea);
  const seg3 = rule.camera_phrases[0];
  const seg4 = rule.lighting_phrases[0];
  const seg5 = `${rule.subject_position}, ${rule.negative_space}`;
  const seg6 = rule.style_tags.join(', ');

  return [seg1, seg2, seg3, seg4, seg5, seg6].join(', ');
}
