  ...
  try {
    ...
    if (match) {
      try {
        ...
        console.error('【AIレスポンス生データ（抽出後）】\n', match[0]);
        console.error('【AIレスポンス生データ（元テキスト）】\n', text);
        throw new Error('AIからのレスポンスをJSONとして解析できませんでした。もう一度試してください。');
      }
    } else {
      console.error('【AIレスポンス生データ（JSON部分なし）】\n', text);
      throw new Error('AIのレスポンスにJSON部分が見つかりませんでした。もう一度試してください。');
    }
  }
